...
// GetDonationsPaused calls CrisisPool.donationsPaused() — returns the live paused state.
func (c *Client) GetDonationsPaused(ctx context.Context, poolAddr common.Address) (bool, error) {
	contract := bind.NewBoundContract(poolAddr, c.PoolABI, c.Eth, c.Eth, c.Eth)
	var out []interface{}
	err := contract.Call(&bind.CallOpts{Context: ctx}, &out, "donationsPaused")
	if err != nil {
